...
struct TrafficLightMapView: View {
    ...
    var body: some View {
        ForEach(self.model.trafficCells) { trafficCell in
            VStack {
                Circle()
                    .fill(Color.red.opacity((trafficCell.trafficLight.color == .red || trafficCell.trafficLight.color == .prepare) ? 1.0 : 0.15))
                    .frame(width: 25, height: 25)

                Circle()
                    .fill(Color.yellow.opacity((trafficCell.trafficLight.color == .yellow || trafficCell.trafficLight.color == .prepare) ? 1.0 : 0.15))
                    .frame(width: 25, height: 25)

                Circle()
                    .fill(Color.green.opacity(trafficCell.trafficLight.color == .green ? 1.0 : 0.15))
                    .frame(width: 25, height: 25)
            }
            .padding(6)
